...
pub fn refresh_angular_scaling(state: &Value, cache: &SharedAngularScaling) {
    ...
    *cache.lock().unwrap() = AngularScaling { pins, scale_default };
}
...
fn angular_scaling_process(x: f64, y: f64, pins: &[Pin], scale_default: f64) -> (f64, f64) {
    ...
    let scale = interp(val, &[eye_angle_lower, eye_angles[upper_i]], &[lower_scale, upper_scale]);
...
/// `AngularScalingConfig.process`), and appends it to `gaze_path` for the
/// operator view's gaze overlay.
pub async fn run(
