...
use skia_safe::gpu::{self, BackendRenderTarget, DirectContext, direct_contexts, surfaces};
...
impl SkiaOffscreen {
  ...
  pub unsafe fn new(base: &VulkanBase, offscreen: &OffscreenTarget) -> Result<Self> {
    ...
    let backend_context = unsafe {
      skia_vk::BackendContext::new_builder(
        ...
        (
          base.queue.as_raw() as skia_vk::Queue,
          base.queue_family_index as usize,
        ),
        &get_proc,
        None,
      )
    ...
    let backend_render_target = gpu::backend_render_targets::make_vk(
      (
        offscreen.extent.width as i32,
        offscreen.extent.height as i32,
      ),
      &unsafe {
  ...
  pub fn set_layout(&mut self, layout: vk::ImageLayout) {
    gpu::backend_render_targets::set_vk_image_layout(
      &mut self.backend_render_target,
      to_skia_layout(layout),
    );
  }
