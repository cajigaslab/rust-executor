...
impl SwapchainWindow {
    pub fn new(base: &VulkanBase, window: winit::window::Window, surface: vk::SurfaceKHR) -> Result<Self> {
            ...
            .map(|command_buffer| -> Result<FrameSync> {
                Ok(FrameSync {
                    ...
                    image_available: unsafe {
                        base.device.create_semaphore(&vk::SemaphoreCreateInfo::default(), None)?
                    },
    ...
    pub fn begin_frame(&mut self, base: &VulkanBase) -> Result<FrameCtx> {
        ...
        if image_fence != vk::Fence::null() {
            unsafe { base.device.wait_for_fences(&[image_fence], true, u64::MAX)? };
        }
    ...
    pub fn end_frame(&mut self, base: &VulkanBase, image_index: u32) -> Result<()> {
        ...
        let (command_buffer, image_available, render_finished, in_flight) =
            (frame.command_buffer, frame.image_available, frame.render_finished, frame.in_flight);
        ...
        unsafe { base.device.queue_submit(base.queue, &[submit_info], in_flight)? };
        ...
        let present_result = unsafe { self.swapchain_loader.queue_present(base.queue, &present_info) };
